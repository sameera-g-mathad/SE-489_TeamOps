...
class Model(HConfig):
    ...
    def compute_metrics(self, eval_pred: EvalPrediction) -> dict:
        ...
        report = classification_report(
            labels,
            predictions,
            target_names=list(self._target2label.keys()),
            output_dict=True,
        )
        self._log.info("Classification report saved.")
        with open("reports/classification_report.txt", "w") as report_file:
            report_file.write(str(report))

        cm = confusion_matrix(
            labels, predictions, labels=list(self._target2label.values())
        )
        disp = ConfusionMatrixDisplay(
            confusion_matrix=cm, display_labels=list(self._target2label.keys())
        )
    ...
    def train(self):
        ...
        # We want cml to always train the model, so we don't check if the model already exists.
        # if (
...
# Make sure cml workflow will run this.
model = Model()
model.train()  # train the model again.
